...
Motor::Motor(int pwmPin, int dirPin, int brakePin, int encAPin, int encBPin,
             ...
             float encoderPPR)
    : _pwmPin(pwmPin), _dirPin(dirPin), _brakePin(brakePin), _encAPin(encAPin),
      _encBPin(encBPin), _pwmChannel(pwmChannel), _pwmFrequency(pwmFrequency),
      _pwmResolution(pwmResolution), _encoderPPR(encoderPPR),
      _lastEncoderCount(0), _currentSpeed_rad_s(0.0f),
      _previousSpeed_rad_s(0.0f), _currentAcceleration_rad_ss(0.0f),
      _brakeActive(false) {
...
void Motor::setEffort(float effort) {
  if (_brakeActive)
    return; // Do not run motor if brake is active
