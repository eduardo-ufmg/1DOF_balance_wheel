...
def fit_and_plot_pwm_speed(filepath):
    ...
    try:
        ...
        if "PWM" not in df.columns or "Speed (rad/s)" not in df.columns:
            print(
                f"Error: The file {filepath} must contain 'PWM' and 'Speed (rad/s)' columns."
            )
    ...
    if (
        len(pwm_values) < 2
        or len(speed_values) < 2
        or len(pwm_values) != len(speed_values)
    ):
        print(
            "Not enough data points or mismatched data lengths to perform a linear fit."
        )
        return
    ...
    try:
        ...
        if (pwm_values.max() - pwm_values.min()) != 0:
            slope_guess = (speed_values.max() - speed_values.min()) / (
        ...
        params, covariance = curve_fit(
            linear_model, pwm_values, speed_values, p0=[slope_guess, intercept_guess]
        )
        Kw, intercept = params
